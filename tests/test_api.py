...
def test_exposed_objects(
    ...
    """All public objects in the internal API are exposed under `auctus_search`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in auctus_search.__all__ or not hasattr(auctus_search, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in auctus_search.__all__:
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("auctus_search")
